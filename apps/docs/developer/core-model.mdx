...
Secret material is private auth state.

It answers:

- which private values exist for this workspace actor?
- which source bindings refer to them?
- which plugin-owned auth configs use those secret refs?

Secret material is actor-scoped by default because two users in the same workspace may connect different identities to the same source.
...
The same is true for auth behavior:

- core owns secret storage and private state plumbing
- plugins own auth schemas, request shaping, OAuth flows, and refresh logic

...
- core owns secret material storage, actor scoping, and private state plumbing
- plugins own auth schemas and auth behavior for each source kind

## Auth ownership

Secret material is scoped to:
...
Plugins may distinguish where auth is used.
...
This is why plugin-owned source auth is more than a single token field.
...
The control plane stores secret refs, not raw secret values inline on the source or plugin-owned auth configs.

## Interactive auth sessions

Interactive auth, such as OAuth flows, is modeled separately from stored secret material.
...
This is why executor keeps auth sessions separate from stored secret material.
...
That is broader than sources and should be reusable by future plugins, tool providers, and code bundles.

## Why auth is separated from recipes

Recipes are shareable integration definitions.
Secret material is private installation-specific state.

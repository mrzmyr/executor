...
That includes auth behavior:

- plugin-owned auth config
- plugin-owned OAuth flows and refresh logic
- core-owned secret material services
...
- auth still uses the same secret material and private-state plumbing
- auth behavior itself stays plugin-owned
...
Those should mostly be adapter and secret-material-layer work, not another persistence redesign.
...
That is the real reason the system is shaped around adapters, recipes, revisions, operations, and actor-scoped private state.

  ...
  it.scoped("supports blocking URL mode elicitation from URL source", () =>
    Effect.gen(function* () {
      ...
      expect(elicitations).toEqual([{
        mode: "url",
        message: "Open the URL to approve from-url-blocking",
      }]);
    }),
  );

  it.scoped("supports non-blocking URL retry elicitation from URL source", () =>
    Effect.gen(function* () {
      ...
      expect(elicitations).toEqual([{
        mode: "url",
        message: "Authorize and then retry for from-url-retry",
      }]);
    }),
  );
});
